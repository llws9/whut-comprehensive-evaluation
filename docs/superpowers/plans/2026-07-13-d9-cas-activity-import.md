...
- The port intentionally does not expose a JDBC connection handle. Same-MySQL-session correctness is guaranteed by call placement: `tryAcquire(...)`, duplicate checks, row processing, and the registered `afterCompletion` `release(...)` callback must all run inside the same `transactionOperations.execute(...)` thread using the same Spring transaction-managed `JdbcTemplate`/`DataSource` bean. Do not call `release(...)` after the transaction callback has returned, from `@Async`, from a new propagation boundary, or through a different `DataSource`; those paths cannot guarantee `RELEASE_LOCK` uses the session that acquired `GET_LOCK`.
...
- when synchronization is active, service tests must verify that release is registered before duplicate check/row processing completes, the normal `finally` path does not release early, manually invoking the registered `afterCompletion` releases exactly once, and an exception from that `afterCompletion` release path is caught and logged instead of propagating to the caller.
...
- Lock orchestration follows D-8 with one explicit D-9 hardening: enter `transactionOperations.execute(...)`, acquire lock inside that transaction callback using the request transaction-bound connection, map `tryAcquire == false` to `ConflictException("同一活动批次正在导入，请稍后重试")`, let `DataAccessException` from `tryAcquire` propagate, register release through `TransactionSynchronizationManager` when synchronization is active, run authoritative duplicate check, process rows, and release through the same transaction-managed `JdbcTemplate`/`DataSource` in `afterCompletion` or in `finally` if synchronization registration did not happen. Because Spring keeps transaction resources bound while invoking transaction synchronizations, the `afterCompletion` callback must run on the same thread and must not delegate to another executor or transaction propagation. If release happens before the transaction outcome is committed, a release `DataAccessException` may still affect the request outcome; if release happens from `afterCompletion`, catch and log the release failure at WARN level so it does not replace an already committed successful response with `500 / SYS-5000 / 数据访问异常，请稍后重试`.
...
- Total recalculation must copy the D-7/D-8 category mapping exactly: initialize `moral`, `intellectual`, `physical`, and `labor` to scaled zero; for every `selectTotals(finalRecordId)` row, scale null to zero and map `MORAL` to `moral`, `INTELLECTUAL` to `intellectual`, `SPORTS` to `physical`, and `LABOR` to `labor`; throw `DataIntegrityViolationException("unsupported final record category")` or another Spring `DataAccessException` for any unexpected category so the existing handler maps it to `500 / SYS-5000 / 数据访问异常，请稍后重试`; do not use `ConflictException` or expose the raw category value to clients for this internal data-integrity failure. Then set `grand = moral + intellectual + physical + labor` before `updateTotals(...)`.
...
- unexpected category totals throw a Spring `DataAccessException` such as `DataIntegrityViolationException("unsupported final record category")`, do not expose the raw category value, and never map to `ConflictException` / `409 / BIZ-4090`.
...
- method parameters must use `@RequestParam(value = "...", required = false)` for all six multipart/request fields: `file`, `title`, `itemCode`, `scoreValue`, `heldAt`, and `academicYear`. Do not use default-required `@RequestParam("...")`, because Spring's missing-part/missing-parameter exceptions would bypass the frozen `VAL-4001` response messages;

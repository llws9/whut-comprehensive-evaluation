...
| `classes` | no | May be sent as repeated query params (`classes=CS2201&classes=CS2202`) or comma-separated tokens (`classes=CS2201,CS2202`). The controller obtains the raw value list with servlet/MVC semantics equivalent to `HttpServletRequest.getParameterValues("classes")`, preserving the query-string occurrence order, and passes that raw list unchanged into `FinalScoreExportQuery`. `FinalScoreExportQuery` owns splitting each value by comma, trimming, dropping blanks, de-duplicating by first appearance in the flattened trimmed token sequence, and enforcing the `500` normalized-token limit. If all tokens are blank, `classes` is equivalent to absent and does not force `404`. More than `500` normalized tokens fail with `400 / VAL-4001` and message `classes 参数过多`. Non-blank tokens match current primary CLASS by exact, case-sensitive `org_unit.unit_code` or `org_unit.unit_name`. Unknown values are not request errors; they produce no matching rows and therefore `404` if nothing else matches. |

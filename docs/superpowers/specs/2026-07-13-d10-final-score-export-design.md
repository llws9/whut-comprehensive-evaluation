...
|---|---:|---|
| `academicYear` | yes | Trimmed, non-blank, must match `yyyy-yyyy`, and second year must equal first year + 1. D-10 does not perform academic-year dictionary, current-year, future-year, or historical-range validation; a format-valid year with no matching authorized records returns the no-data `404 / RES-4040` branch. |
...
| `classes` | no | May be sent as repeated query params (`classes=CS2201&classes=CS2202`) or comma-separated tokens (`classes=CS2201,CS2202`). The controller must read the raw request parameter array/list with servlet semantics equivalent to `HttpServletRequest.getParameterValues("classes")`, preserving query-string occurrence order and the original un-split raw values. It must not use MVC binding forms or converters that automatically split comma-separated strings before `FinalScoreExportQuery` receives them. The controller passes the raw list unchanged into `FinalScoreExportQuery`; `FinalScoreExportQuery` owns splitting each raw value by comma, trimming, dropping blanks, de-duplicating by first appearance in the flattened trimmed token sequence, and enforcing the frozen `500` normalized-token limit. If all tokens are blank, `classes` is equivalent to absent and does not force `404`. More than `500` normalized tokens fail with `400 / VAL-4001` and message `classes 参数过多`. Non-blank tokens match current primary CLASS by exact, case-sensitive `org_unit.unit_code` or `org_unit.unit_name`. Unknown values are not request errors; they produce no matching rows and therefore `404` if nothing else matches. |
...
D-10 exports current committed data at request execution time. It does not promise byte-level idempotence across repeated calls with the same parameters: rows, statuses, totals, and timestamps may change between requests as final records are submitted or confirmed. Deterministic ordering only makes a single result stable for the rows returned by that query; consistent point-in-time snapshots, export caching, and repeatable async artifacts are out of scope for D-10.
...
| I | `状态` | `status` | text, raw enum value `SUBMITTED` or `CONFIRMED` |
...
- D-10 caps synchronous workbook generation at `20_000` export rows. Define a single application-layer constant on `FinalScoreExportApplicationService`, for example `public static final int MAX_SYNC_EXPORT_ROWS = 20_000`, and derive the repository probe limit as `MAX_SYNC_EXPORT_ROWS + 1`; do not scatter hard-coded `20_000`/`20_001` values across service and application-service tests. Application-service tests must reference the service constant or a service-owned accessor rather than duplicating the numeric values. The repository receives the derived `limit` method parameter and must not directly reference the application-layer constant. Repository tests verify the repository honors the `limit` method argument with small local limits such as `2` and `3`; they must not import or depend on `FinalScoreExportApplicationService.MAX_SYNC_EXPORT_ROWS`. The repository must enforce the detection mechanism with a bounded export-row query that applies all filters, scope predicates, and the deterministic `ORDER BY` first, then applies `LIMIT #{limit}`. Do not use an unbounded full read followed by an in-memory size check. The service asks for up to `MAX_SYNC_EXPORT_ROWS + 1` authorized rows; if the returned list size is greater than `MAX_SYNC_EXPORT_ROWS`, the service fails before workbook allocation with `FinalScoreExportGenerationException("Excel 生成失败")`, returning the existing `503 / EXT-5033` response branch. This intentionally merges "too many rows for the synchronous MVP" and workbook-generation failures into the same frozen public response because the D delivery contract only exposes `EXT-5033 / Excel 生成失败` for export generation failure. D-10 must not introduce a distinct public error code, HTTP status, or message for row-cap overflow; the follow-up path for larger exports is an out-of-scope async/export-job design.
- D-10 does not add endpoint-specific timeout configuration. It relies on the existing application and gateway HTTP timeout settings. The `20_000` row cap is the D-10 guardrail to keep the synchronous path within ordinary school-cohort use; tests freeze the cap and overflow response, while production response-time SLOs and async export behavior remain follow-up operational/design work.
...
- Convert `classes` by reading the raw request parameter array/list equivalent to `HttpServletRequest.getParameterValues("classes")` and passing those original un-split values to the application query. Splitting, trimming, de-duplication, and the `500` token cap live in `FinalScoreExportQuery` so MVC and service tests share behavior.
...
- The academic-year end must equal start + 1. D-10 intentionally does not validate the academic year against a dictionary, current clock year, or configured range; unknown but format-valid academic years are valid requests and produce `404 / RES-4040` only if no authorized records match.
...
The controller converts HTTP parameters into `FinalScoreExportQuery` only after raw request-shape checks pass. It must not pre-split, trim, de-duplicate, or cap `classes`; those semantic steps live in `FinalScoreExportQuery` so MVC and direct query/service tests exercise the same normalization logic. Direct query/service tests cover semantic validation; controller/WebMvc tests cover raw `classes` preservation, `pageNo`/`pageSize` presence, and repeated single-value request-shape errors.
...
Although row-cap overflow and workbook writer failure share the same public response, the service must log distinct internal context before throwing: row-cap overflow logs the requested academic year, normalized filters, returned row count, and `MAX_SYNC_EXPORT_ROWS`; workbook writer failure logs the requested academic year, row count, and original exception type/message. These logs must not change the public `503 / EXT-5033 / Excel 生成失败` contract.

## Tests

Spec-phase acceptance tests for the implementation plan:

- Query normalization trims `academicYear`, rejects missing/invalid `academicYear`, verifies `2025-2026` style consecutive-year validation without rejecting unknown but format-valid years by dictionary/range, treats blank `status` as absent, rejects lowercase/non-exact `status` values and `DRAFT`, treats blank `grade` as absent, normalizes repeated `classes=CS2201&classes=CS2202`, comma-separated `classes=CS2201,CS2202`, and mixed `classes=A,B&classes=B,C` using servlet/MVC `getParameterValues("classes")` query-string occurrence order, drops blank class tokens, de-duplicates by first appearance in that order, rejects more than `500` normalized class tokens with `classes 参数过多`, treats `classes=,,` or `classes=&classes= ` as an absent class filter, and verifies a missing `classes` key and present-but-all-blank `classes` key produce the same empty immutable list representation.
...
- Controller/WebMvc tests cover `classes=A,B&classes=B,C` through the HTTP layer and verify the service receives a query normalized to `[A, B, C]`, proving the controller preserved raw un-split parameter values and delegated splitting/de-duplication to `FinalScoreExportQuery`.
...
- Workbook writer creates exactly one worksheet named `final-scores`, freezes the first row, creates the exact header row, writes `finalRecordId` as a text cell to preserve 64-bit identifiers, writes numeric total cells with `RoundingMode.HALF_UP`, writes blank cells for unexpected null totals, writes blank E-H cells for null `gradeCode`/`gradeName`/`classCode`/`className` without shifting columns, writes UTC timestamp text truncated to seconds, emits no formulas, applies readable column widths or autosizing with the minimum-width check above, and returns the fixed xlsx content type.
- Workbook writer tests must include a representative 1-2 row sample and assert the exact data-region row count plus A-P cell mapping for every data row. That assertion must verify each column's value and cell type against the frozen table above, including text `finalRecordId`, raw enum `status`, numeric totals at scale 2, blank E-H cells when organization fields are null, blank timestamp cells when source timestamps are null, and UTC timestamp text truncated to seconds when timestamps are present.
...
- Application service logs distinct internal context for row-cap overflow versus workbook writer failure while preserving the same public `503 / EXT-5033` response.
- Application service or controller tests document current-snapshot semantics: repeated calls with the same parameters query current data each time and are not required to return byte-identical workbook content if final-record data changes between calls.
...
- Repository row-limit tests use small local `limit` method arguments and assert the SQL returns no more than that argument after filters, scope predicates, and ordering are applied; they must not depend on `FinalScoreExportApplicationService.MAX_SYNC_EXPORT_ROWS`.

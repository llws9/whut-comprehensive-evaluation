...
The guard table name and shape are fixed for D-10: `CREATE TEMPORARY TABLE IF NOT EXISTS d_seed_collision_guard (id BIGINT NOT NULL PRIMARY KEY)`, followed by `DELETE FROM d_seed_collision_guard` and `INSERT INTO d_seed_collision_guard (id) VALUES (1)`. Each collision check must use `INSERT INTO d_seed_collision_guard (id) SELECT 1 ...` so a matching collision attempts to insert the existing guard primary key and raises a duplicate-key error. The expected signature must include all frozen semantic columns: `assignment_id`, `permission_code`, `scope_type`, `org_unit_id`, `category_code`, `item_code`, `expression_json`, `priority`, and `status`; `created_at` is excluded because reruns may preserve the original timestamp. The guard SQL must be executable in both MySQL and the existing H2 MySQL-mode seed tests, so it must not depend on MySQL-only JSON extraction functions such as `JSON_EXTRACT` or `JSON_UNQUOTE`. Compare `expression_json` by normalizing its text representation, for example by removing insignificant spaces from `CAST(expression_json AS CHAR)` before comparing it with compact literals such as `{"scoreRole":"counselor"}`, `{"scoreRole":"college_reviewer"}`, and `{"superAdmin":true}`. For example, `8023` must check the non-JSON columns plus a normalized `expression_json` text comparison equivalent to `{"scoreRole":"counselor"}`; equivalent JSON/value checks are required for `8024` and `8025`.
...
- `classes` tokens are filter expressions, not lookups that resolve to one organization row. If the same token matches one class's `classCode` and another class's `className`, rows from both classes are included.
...
- Repository executes the accepted `500` normalized `classes` token boundary in an integration test, proving the generated two-`IN` predicate with up to `1000` bound parameters is executable in the supported database path.

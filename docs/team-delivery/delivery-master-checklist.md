...
| 18 | B | D | `D-8/D-9` 对讲座与活动源数据的导入口径 | 联调前置 | 中 | B-9 查询讲座/活动候选项需要依赖 D 组导入后的讲座/活动源数据或至少共享字段契约；当前 D-8 最小讲座导入只写最终成绩明细，不关闭 B-9 的 `lectureId/maxScore/attendanceStatus` 源数据依赖 |
